...
local assets ={
    Asset("ANIM", "anim/tees_pickaxe.zip"),
    Asset("ANIM", "anim/swap_tees_pickaxe.zip"), --<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< swap파일을 로드 하지 않았습니다. 쥐님.
...
local function onequip(inst, owner)
    owner.AnimState:OverrideSymbol("swap_object", "swap_tees_pickaxe", "swap")
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("tees_pickaxe.tex")	
    ...
    inst.AnimState:SetBank("tees_pickaxe")
    inst.AnimState:SetBuild("tees_pickaxe")
	...
	--inst.components.tool:SetAction(ACTIONS.MINE, 1.33)--곡괭이
	...
	inst.components.inventoryitem.imagename = "tees_pickaxe"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/tees_pickaxe.xml" --인벤토리 아이템으로 설정됨
...
return Prefab("tees_pickaxe", fn, assets, prefabs) --YK : prefab 어규먼트
