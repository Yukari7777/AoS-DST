...
local assets ={
    Asset("ANIM", "anim/anan_dagger_hard.zip"),
    Asset("ANIM", "anim/swap_anan_dagger_hard.zip"), --<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< swap파일을 로드 하지 않았습니다. 쥐님.
...
local function onequip(inst, owner)
    owner.AnimState:OverrideSymbol("swap_object", "swap_anan_dagger_hard", "swap")
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("anan_dagger_hard.tex")	
    ...
    inst.AnimState:SetBank("anan_dagger_hard")
    inst.AnimState:SetBuild("anan_dagger_hard")
	...
	-- 무기로 설정. 아래는 피해 설정
	inst.components.weapon:SetRange(1.1) --공격범위
	...
	inst.components.inventoryitem.imagename = "anan_dagger_hard"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/anan_dagger_hard.xml" --인벤토리 아이템으로 설정됨
...
return Prefab("anan_dagger_hard", fn, assets, prefabs) --YK : prefab 어규먼트