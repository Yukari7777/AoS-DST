...
local assets ={
    Asset("ANIM", "anim/tees_pickaxe2.zip"),
    Asset("ANIM", "anim/swap_tees_pickaxe2.zip"), --<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< swap파일을 로드 하지 않았습니다. 쥐님.
...
local function onequip(inst, owner)
    owner.AnimState:OverrideSymbol("swap_object", "swap_tees_pickaxe2", "swap")
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("tees_pickaxe2.tex")	
    ...
    inst.AnimState:SetBank("tees_pickaxe2")
    inst.AnimState:SetBuild("tees_pickaxe2")
	...
	inst.components.tool:SetAction(ACTIONS.MINE, 1.5)--곡괭이
	inst.components.tool:SetAction(ACTIONS.CHOP, 1.5)--도끼
    --]]
	...
	inst.components.inventoryitem.imagename = "tees_pickaxe2"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/tees_pickaxe2.xml" --인벤토리 아이템으로 설정됨
...
return Prefab("tees_pickaxe2", fn, assets, prefabs) --YK : prefab 어규먼트
